      ...
      <div class="col-md-9">
        <div v-for="post in posts" :key="post.id">
          <article
            ...
            v-if="
              post.posted_on == 'Comment' &&
              (post.member_type == 'Admin' ||
                post.member_type == 'Moderator' ||
                post.post_by_id == $store.state.id)
            "
            style="margin-right: 50px"
          >
            <!-- <h2>
              <a :href="'/post-details/' + post.comment_post_id">{{
                post.post_title
              }}</a>
            </h2> -->
            <a
            ...
            <div class="line"></div>
            <div v-if="post.post_by_id != $store.state.id">
              <div
                class="question-report"
                @click="ApproveRejectContent(false, post.id)"
                v-if="
                  post.member_type == 'Admin' || post.member_type == 'Moderator'
                "
              >
                Reject{{ post.member_type }}
              </div>
              <div
